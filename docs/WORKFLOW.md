...
- All web search URLs are collected and stored
...
3. **Hydration** - Research distributions, can reference base attrs
...
**CLI:** `entropy scenario "AI tool announcement" -p surgeons.yaml -a agents.json -n network.json -o scenario.yaml`
...
│   │   ├── hydrator_utils.py       # Parsing helpers, schemas
...
│   │
│   └── validator/
│       ├── __init__.py             # validate_spec()
│       ├── syntactic.py            # ERROR checks
│       ├── semantic.py             # WARNING checks
│       └── fixer.py                # Auto-fix
│
├── network/
...
└─────────────────────────────────────────────────────────────────────────┘
  ...
  └────────┬─────────┘
           │
           ├──▶ hydrate_independent() ──▶ distributions
           ├──▶ hydrate_derived() ──▶ formulas
           ├──▶ hydrate_conditional_base() ──▶ base distributions
           └──▶ hydrate_conditional_modifiers() ──▶ modifiers
                            │
                            ▼
                   ┌──────────────────┐
                   │ List[Hydrated    │
...
└─────────────────────────────────────────────────────────────────────────┘
  ...
  └──────────────────┘      │ base attributes  │
                            └────────┬─────────┘
                                     │
         (hydrate, bind, validate as above)
                                     │
                                     ▼
                   ┌──────────────────┐
...
└─────────────────────────────────────────────────────────────────────────┘
  ...
  "AI tool announcement" + surgeons.yaml + agents.json + network.json
         ...
         ├──▶ determine_interaction_model() ──▶ InteractionConfig, SpreadConfig
...
| `reasoning_call()`   | gpt-5              | Reasoning enabled         | Attribute selection, scenario parsing |
| `agentic_research()` | gpt-5 + web_search | Reasoning + web search    | Distribution research                 |
...
| Hydration   | Invalid distribution | hydrator_utils.py | Logged as validation warning      |
| Binding     | Circular dependency  | binder.py         | Raises CircularDependencyError    |
| Validation  | Type mismatch        | syntactic.py      | ERROR - blocks sampling           |
| Validation  | Option mismatch      | semantic.py       | WARNING - auto-fixable            |
